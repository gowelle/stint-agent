...
async function runTest(): Promise<void> {
    ...
    try {
        if (os === 'win32') {
            ...
            const result = await execAsync(`powershell -ExecutionPolicy Bypass -File "${scriptPath}" -InitialVersion ${INITIAL_VERSION} -TargetVersion ${TARGET_VERSION}`);
            if (result.stderr) console.error(result.stderr);
        } else {
            const scriptPath = join(__dirname, 'test-update-unix.sh');
            const result = await execAsync(`bash "${scriptPath}" ${INITIAL_VERSION} ${TARGET_VERSION}`);
            if (result.stderr) console.error(result.stderr);
        }

        console.log('\n✨ All tests completed successfully');
    } catch (error) {
        console.error('\n❌ Test failed:', (error as ExecException).message);
